...
type InstagramDeletionSignedRequest = Record<string, unknown> & {
  issued_at?: number;
  user_id?: string;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = verifyMetaSignedRequest<InstagramDeletionSignedRequest>(
      signedRequest,
    ...
    console.error("Instagram data deletion callback failed", {
      code: "instagram_data_deletion_failed",
    });

    return NextResponse.json(
      { message: "Unable to process data deletion request.", code: "instagram_data_deletion_failed" },
      { status: 500 },
    );
  }
}

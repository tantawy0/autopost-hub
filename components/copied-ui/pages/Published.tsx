...
export default function Published() {
  ...
  return <div className="space-y-5"><div className="flex items-end justify-between"><div><h1 className="font-display text-3xl font-bold tracking-tight">Published</h1><p className="text-sm text-muted-foreground">Recent posts and their performance.</p></div><Button onClick={() => setShowFailed((value) => !value)} variant="outline" size="sm" className="border-border"><Filter className="mr-1.5 h-3.5 w-3.5" /> {showFailed ? "Hide failed" : "Show all"}</Button></div>
    ...
    {!visiblePosts.length ? <div className="glass rounded-2xl p-12 text-center text-sm text-muted-foreground">No published posts yet.</div> : null}
  </div>;
}

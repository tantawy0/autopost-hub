...
export default function Queue() {
  ...
  const publish = async (post: UiPost) => {
    try { await publishPostClient(post.id, await listSelectableDestinations()); toast.success("Publishing complete"); await load(); }
  ...
  return <div className="space-y-5">
    <div className="flex items-end justify-between"><div><h1 className="font-display text-3xl font-bold tracking-tight">Queue</h1><p className="text-sm text-muted-foreground">Drafts, scheduled and failed posts in one place.</p></div><div className="flex gap-2"><Button variant="outline" size="sm" className="border-border"><Filter className="mr-1.5 h-3.5 w-3.5" /> Filter</Button><Button asChild className="bg-gradient-primary text-primary-foreground shadow-glow"><Link href="/create"><Plus className="mr-1 h-4 w-4" /> New post</Link></Button></div></div>
